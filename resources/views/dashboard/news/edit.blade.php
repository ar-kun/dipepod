...
<div class="col-lg-10">
  <form action="/dashboard/news/{{ $news->slug }}" method="post" class="mb-5" enctype="multipart/form-data">
    ...
    <div class="mb-3">
      ...
      <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" autofocus value="{{ old('title',$news->title) }}">
      @error('title')
    ...
    <div class="mb-3">
      ...
      <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" value="{{ old('slug',$news->slug) }}" readonly hidden>
      @error('slug')
          <div class="invalid-feedback">
            {{ $message }}
          </div>
    ...
    <div class="mb-3">
      <label for="slug" class="form-label">Category</label>
        <select class="form-select" name="newscategory_id">
          @foreach ($categories as $category)
              @if (old('newscategory_id',$news->newscategory_id)== $category->id)
