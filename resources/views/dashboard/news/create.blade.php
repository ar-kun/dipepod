...
<div class="col-lg-10">
  <form action="/dashboard/news" method="post" class="mb-5" enctype="multipart/form-data">
    ...
    <div class="mb-3">
      ...
      <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" disabled readonly value="{{ old('slug') }}" hidden>
      @error('slug')
          <div class="invalid-feedback">
            {{ $message }}
          </div>
      ...
      <label for="slug" class="form-label">Category</label>
        <select class="form-select" name="newscategory_id">
          @foreach ($categories as $category)
              @if (old('newscategory_id')== $category->id)
